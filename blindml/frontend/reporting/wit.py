...
# converts a list of tf.Example protos into an np array
# note that we only handle numeric values
def examples_to_array(
    examples: List[tf.train.Example], X_cols: List[str]
) -> np.ndarray:
    ...
    for ex in examples:
        for feat_name, feat_vals in ex.features.feature.items():
            ...
            _field_descriptor, field = feat_vals.ListFields()[0]
            assert len(field.value) == 1, "we don't support list fields"
            data[feat_name].append(field.value[0])
